  ...
  handleReply() {
    ...
    this.getTopic().doReply(this.state.newReply)
      .then(() => {
        ...
        this.props.appState.notify({ message: 'success' })
      })
      .catch(() => {
        this.props.appState.notify({ message: 'failure' })
      })
    // axios.post('/api/')
  }
      ...
      <div>
        ...
        <Paper elevation={4} className={classes.replies}>
          <header className={classes.replyHeader}>
            ...
            <span>{`最新回复 ${formatDate(topic.last_reply_at, 'yyyearmmonthddday')}`}</span>
          </header>
          {
            ...
            !user.isLogin ?
              (
                <section className={classes.notLoginButton}>
                  <Button raised color="accent" onClick={this.goToLogin}>log in to reply</Button>
                </section>
              ) :
              null
          }
